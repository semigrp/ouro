...
const contextQuerySchema = readContract("negura-context-query.v1.schema.json");
...
const validateEvidenceContract = ajv.compile(readContract("negura-register-evidence.v1.schema.json"));
...
export function assertRunRequest(value: unknown): asserts value is RunRequestV1 {
  ...
  if (request.experiment) assertOwnedPinnedRef(request.experiment, "negura", "experiment");
  for (const root of request.contextQuery.roots) assertPinnedRef(root, "Context query root");
  if (request.procedure.definition) {
    assertOwnedPinnedRef(request.procedure.definition, "negura", "procedure");
  }
  ...
  for (const assessment of request.evidence?.assessments ?? []) {
    assertOwnedPinnedRef(assessment.claim, "negura", "claim");
  }
}
...
export function assertEvidenceCommand(value: unknown): asserts value is RegisterEvidenceCommandV1 {
  if (!validateEvidenceContract(value)) {
    throw new Error(
      `Invalid negura.register-evidence/v1: ${JSON.stringify(validateEvidenceContract.errors)}`,
    );
  }
