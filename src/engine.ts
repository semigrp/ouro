...
import type { NeguraGateway } from "./adapters/negura.js";
...
export class OuroEngine {
  ...
  async run(value: unknown): Promise<RunRecord> {
    ...
    return this.repository.withWriterLock(async () => {
      ...
      const bundle = await this.negura.queryContext(request.contextQuery);
      ...
      if (shouldRegisterEvidence(request, succeeded)) {
        ...
        const entry: NeguraOutboxEntry = {
          schema: "ouro.negura-outbox/v1",
          id: commandId,
          command: {
        ...
        store.neguraOutbox[commandId] = entry;
      }
      await this.repository.save(store);
  ...
  async flushNeguraOutbox(): Promise<FlushResult> {
    return this.repository.withWriterLock(async () => {
      const store = await this.repository.load();
      assertValidStore(store);
  ...
  private async flushLocked(store: OuroStore): Promise<FlushResult> {
    let attempted = 0;
    let delivered = 0;
    for (const entry of Object.values(store.neguraOutbox).filter((item) => item.status === "pending")) {
      assertEvidenceCommand(entry.command);
      attempted += 1;
      entry.attempts += 1;
      ...
      try {
        const evidence = await this.negura.registerEvidence(entry.command);
        assertPinnedRef(evidence, "Registered Negura Evidence");
...
function assertContextBundleMatches(bundle: ContextBundleV1, request: RunRequestV1): void {
  if (bundle.schema !== "negura.context-bundle/v1") throw new Error("Invalid Negura ContextBundle schema");
  ...
  if (digestJson(contextBundleDigestPayload(bundle)) !== bundle.digest) {
    throw new Error("Negura ContextBundle digest mismatch");
  }
