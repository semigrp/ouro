...
| ExperimentDefinition and ProcedureDefinition remain Boros refs | RunRequest contract and ADR 0001 | Pass |
...
| Gate result is Run-specific, not a Boros Decision | Gate model and ADR 0001 | Pass |
...
| Evidence command survives delivery failure and replays idempotently | Outbox test and real Boros replay test | Pass |
...
| Cross-repository golden path runs in CI | `BOROS_ROOT` test with separate Boros checkout | Pass |
