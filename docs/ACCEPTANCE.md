...
| ExperimentDefinition and ProcedureDefinition remain Negura refs | RunRequest contract and ADR 0001 | Pass |
...
| Gate result is Run-specific, not a Negura Decision | Gate model and ADR 0001 | Pass |
...
| Evidence command survives delivery failure and replays idempotently | Outbox test and real Negura replay test | Pass |
...
| Cross-repository golden path runs in CI | `NEGURA_ROOT` test with separate Negura checkout | Pass |
