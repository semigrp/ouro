...
The agent loop is split into three bounded contexts. Negura owns durable knowledge meaning,
...
Ouro queries Negura for a pinned ContextBundle before execution. Ouro stores the bundle reference,
digest, ontology reference, query, and selected revisions as reproducibility inputs, not as a
knowledge source of truth. Evidence registration is a Negura-owned command. Ouro records that
command in a durable outbox keyed by the terminal execution event and may retry delivery. Negura
provides receiver-side idempotency.

ExperimentDefinition and ProcedureDefinition remain Negura objects. Ouro stores only their pinned
...
can identify its Work projection, Plan, Task, Negura context and definitions, artifact commit and
