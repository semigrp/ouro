...
import { StaticNeguraGateway } from "../src/adapters/negura.js";
...
test("receiver and producer contract fixtures validate", async () => {
  ...
  const evidence = JSON.parse(
    await readFile(join(contracts, "fixtures", "negura-register-evidence.valid.json"), "utf8"),
  ) as unknown;
...
test("golden path traces pinned inputs, gates, outputs, telemetry, and Evidence", async () => {
  ...
  try {
    ...
    assert.equal(Object.values(store.neguraOutbox)[0]?.status, "delivered");
    ...
    const replay = await fixture.engine.flushNeguraOutbox();
...
test("Fukuro and Negura delivery outages never roll back a completed Run", async () => {
  const fixture = await createFixture({ procedure: successProcedure() });
  ...
  try {
    ...
    assert.equal(Object.values(store.neguraOutbox)[0]?.status, "pending");
    ...
    const flushed = await fixture.engine.flushNeguraOutbox();
...
async function createFixture(options: {
  ...
  const request: RunRequestV1 = {
    ...
    contextQuery: query,
    procedure: {
      definition: { system: "negura", type: "procedure", id: "PROC-TEST", version: "1" },
      artifact: {
        system: "github",
        type: "file",
...
function fixtureQuery(): ContextQueryV1 {
  return {
    ...
    roots: [{ system: "negura", type: "experiment", id: "EXP-TEST", version: "1" }],
...
test("prepare: find-or-create is idempotent and the saved request is pinned", async () => {
  ...
  try {
    // a stub negura CLI: `find` consults a state file, create commands append to it
    const stub = join(dir, "stub-negura.js");
    await writeFile(
      stub,
      `
