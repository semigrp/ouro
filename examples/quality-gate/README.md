...
knowledge roots in Bouro and telemetry piped into Fukuro. This is the smallest real (non-demo)
...
2. **Root the run in Bouro knowledge** (once per question):

   ```bash
   bouro claim      --title "Quality gates decide HEAD health" --statement "..."
   bouro question   --title "Does <repo> HEAD pass its quality gates?" --question "..." --closure-rule "An Ouro run's exit_code gate decides."
   bouro hypothesis --title "<repo> HEAD is gate-clean" --claim CLM-n --question QST-n --closes-when "..."
   bouro experiment --title "Run <repo> quality gates under Ouro" --question QST-n --hypothesis HYP-n --success "Every declared command exits 0."
   bouro procedure  --title "quality-gate procedure" --purpose "Run a declared command list and gate on the first failure." \
...
node dist/bin/ouro.js bouro flush   # deliver (or replay) the Evidence command
```
...
- **Bouro**: one Evidence entry (idempotency-keyed) answering the Experiment's Question.
